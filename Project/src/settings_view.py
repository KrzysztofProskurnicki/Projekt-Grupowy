...
from styles import (
    DARK_BG, CARD_BG, HOVER_BG, BORDER_COLOR,
...
class SettingsView(QWidget):
    ...
    def __init__(self, settings_service):
        ...
        self._btn_groups = {}  # key -> list of (value, button)
        self.init_ui()
    ...
    def _create_option_row(self, options, current_value, on_select, group_key):
        """Helper to create a row of toggle buttons."""
        layout = QHBoxLayout()
    ...
    def _update_btn_styles(self, group_key, active_value):
        """Update styles for a group of buttons to reflect active state."""
    ...
    def _build_auto_lock(self):
        ...
        desc.setStyleSheet(f"font-size: 14px; color: {TEXT_SECONDARY}; border: none;")
        layout.addWidget(desc)

        options = [
            ("Off", 0), ("1 min", 1), ("5 min", 5),
        ...
        btn_row = self._create_option_row(options, self._settings.auto_lock_minutes, on_select, 'auto_lock')
    ...
    def _build_clipboard_clear(self):
        ...
        options = [
            ("Off", 0), ("10 sec", 10), ("30 sec", 30),
            ("60 sec", 60), ("120 sec", 120)
        ]
        ...
        btn_row = self._create_option_row(options, self._settings.clipboard_clear_seconds, on_select, 'clipboard_clear')
        layout.addLayout(btn_row)
        self.layout_main.addWidget(card)
    ...
    def _build_theme(self):
        ...
        update_theme_btns(self._settings.theme)
    ...
    def _build_font_size(self):
        ...
        self.size_lbl.setStyleSheet(f"font-size: 16px; font-weight: bold; color: {TEXT_PRIMARY}; border:none;")
        ...
        self.preview_lbl.setStyleSheet(f"font-size: {self._settings.font_size}px; color: {TEXT_PRIMARY}; border: none; margin-top: 10px;")
        layout.addWidget(self.preview_lbl)

        def on_slider_change(val):
            self.size_lbl.setText(f"{val}px")
            self.preview_lbl.setStyleSheet(f"font-size: {val}px; color: {TEXT_PRIMARY}; border: none; margin-top: 10px;")
